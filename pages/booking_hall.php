...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if(empty($_REQUEST['checkin'])||empty($_REQUEST['person'])||$_REQUEST['hall']=="other"||$_REQUEST['cater']=="other")
	{
		...
		header("location:booking_hall.php?err=$err&occasion=$_REQUEST[occasion]&checkin=$_REQUEST[checkin]&person=$_REQUEST[person]&hall=$_REQUEST[hall]&cater=$_REQUEST[cater]&time=$_REQUEST[time]&hours=$_REQUEST[hours]&pay=$_REQUEST[pay]&payment=$_REQUEST[payment]&reg=$_REQUEST[reg]&inErr=$inErr&personErr=$personErr&hallErr=$hallErr&caterErr=$caterErr&timeErr=$timeErr");
	...
	{	
		...
		if ($_REQUEST['checkin'] <= $tomorrow)
		{
						...
						$inErr = "*";
						header("location:booking_hall.php?err=$err&occasion=$_REQUEST[occasion]&checkin=$_REQUEST[checkin]&hall=$_REQUEST[hall]&hours=$_REQUEST[hours]&person=$_REQUEST[person]&cater=$_REQUEST[cater]&time=$_REQUEST[time]&pay=$_REQUEST[pay]&payment=$_REQUEST[payment]&reg=$_REQUEST[reg]");																											
		...
		else if($_REQUEST['confbut'] == "YES")
				{
					...
					else
					{
						if($ch['fldtime']=='AM')
						{
							...
							{
								$err = "the hall is already reserved on that day";
								header("location:booking_hall.php?err=$err&occasion=$_REQUEST[occasion]&checkin=$_REQUEST[checkin]&hall=$_REQUEST[hall]&hours=$_REQUEST[hours]&person=$_REQUEST[person]&cater=$_REQUEST[cater]&time=$_REQUEST[time]");																					
							}
							else
							{
								header("location:register5.php?err=$err&occasion=$_REQUEST[occasion]&checkin=$_REQUEST[checkin]&hall=$_REQUEST[hall]&hours=$_REQUEST[hours]&person=$_REQUEST[person]&cater=$_REQUEST[cater]&pay=$_REQUEST[pay]&payment=$_REQUEST[payment]&reg=$_REQUEST[reg]&time=$_REQUEST[time]");									
							}
						}
			...
			header("location:booking_hall.php");
			
			}
			else
			{
		 	header("location:booking_hall.php?&occasion=$_REQUEST[occasion]&checkin=$_REQUEST[checkin]&hall=$_REQUEST[hall]&hours=$_REQUEST[hours]&person=$_REQUEST[person]&cater=$_REQUEST[cater]&time=$_REQUEST[time]&pay=$_REQUEST[pay]&payment=$_REQUEST[payment]&reg=$_REQUEST[reg]&conf=1");
			}		
		}
	}
...
<hr>
	...
	<?php 
						...
						<form action="booking_hall.php" method="get">
						...
						<br><span id="tab"><br>Occasion:<font color='red'><?php echo $_REQUEST['occassionErr']; ?></font>  <select class="form-control" name="oc">
