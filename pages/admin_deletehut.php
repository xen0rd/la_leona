	  ...
	  die('Could not connect: ' . mysql_error());
			...
			if($_REQUEST['reserve']==$_REQUEST['reg'])
			{
				if($_REQUEST['confbut'] == "YES")
				{
					...
					$result = mysql_query("DELETE FROM tblhut WHERE fldtype ='$_REQUEST[type]' ");		
				...
				{
					header("location:admin_deletehut.php?type=$_REQUEST[type]&form=1&type=$_REQUEST[type]&day=$_REQUEST[day]&night=$_REQUEST[night]&avail=$_REQUEST[avail]&conf=1");
			}		
			}
		mysql_close($con);
				}
...
?>
<form action="admin_deletehut.php" method="post">
