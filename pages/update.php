...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$rest = mysql_query("SELECT fldquantity FROM tblfoodinven WHERE flditemcode = '$prodcodes' ");
...
if(empty($_REQUEST['itemcode'])||empty($_REQUEST['itemname'])||empty($_REQUEST['price'])||empty($_REQUEST['unit']))
{
	...
	header("location:update.php?prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemcodeErr=$itemcodeErr&itemname=$_REQUEST[itemname]&itemnameErr=$itemnameErr&price=$_REQUEST[price]&priceErr=$priceErr&unit=$_REQUEST[unit]&unitErr=$unitErr&add=$add&delete=$delete");
}
else
{
	if($tot > 500)
	{
		...
		header("location:update.php?stocksErr=$stocksErr&prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemname=$_REQUEST[itemname]&add=$add&delete=$delete");
	}
	else if($tot <= 500 )
	{
		$tot = $tot - $delete;
		...
		else
		{
			if($_REQUEST['itemcode']==$_REQUEST['prodcode'])
			{
				...
				{
				$sql="UPDATE tblfoodinven SET flditemcode = '$_REQUEST[itemcode]',fldstatus = '$_REQUEST[status]', flditemname = '$_REQUEST[itemname]',fldprice = '$_REQUEST[price]',fldunit = '$_REQUEST[unit]', fldquantity = '$tot' WHERE flditemcode = '$_REQUEST[prodcode]'";
				mysql_query($sql);
				header("location:foodinventory.php");
				}
				else
				{
					header("location:update.php?prodcode=$_REQUEST[prodcode]&form=1&status=$_REQUEST[status]&tot=$tot&itemcode=$_REQUEST[itemcode]&itemname=$_REQUEST[itemname]&price=$_REQUEST[price]&unit=$_REQUEST[unit]&&add=$add&delete=$delete&conf=1");
				}
			}
			else
			{
				$dupe = mysql_query("SELECT * FROM tblfoodinven WHERE flditemcode='$_POST[itemcode]'") or die (mysql_error());
				...
				if ($num_rows > 0) 
				{
					...
					$itemcodeErr="*";
					header("location:update.php?prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemcodeErr=$itemcodeErr&itemname=$_REQUEST[itemname]&price=$_REQUEST[price]&unit=$_REQUEST[unit]&add=$add&delete=$delete");
				}
				else 
				{
					if($_REQUEST['confbut'] == "YES")
					{
					$sql="UPDATE tblfoodinven SET flditemcode = '$_REQUEST[itemcode]', flditemname = '$_REQUEST[itemname]',fldprice = '$_REQUEST[price]',fldunit = '$_REQUEST[unit]', fldquantity = '$tot' WHERE flditemcode = '$_REQUEST[prodcode]'";
					mysql_query($sql);
					header("location:foodinventory.php");
					}
					else
					{
					header("location:update.php?prodcode=$_REQUEST[prodcode]&form=1&itemcode=$_REQUEST[itemcode]&itemname=$_REQUEST[itemname]&quantity=$_REQUEST[quantity]&price=$_REQUEST[price]&unit=$_REQUEST[unit]&&add=$add&delete=$delete&conf=1");
					}
				}
			}
		}
...
$prodcode=$_REQUEST['prodcode'];
	...
	mysql_select_db("dbresort", $con);
$res = mysql_query("SELECT * FROM tblfoodinven WHERE flditemcode = '$prodcode' ");
...
<?php
	...
	?>
<input  type="hidden" class="form-control" style="width:40%" name="status" value="<?php echo $status; ?>"><span class="colorr"><?php echo $_REQUEST['unitErr']; ?></span><br>
<input  type="hidden" class="form-control" style="width:40%" name="tot" value="<?php echo $_REQUEST['tot']; ?>"><span class="colorr"><?php echo $_REQUEST['unitErr']; ?></span><br>
...
if($_REQUEST['conf']==NULL)
{
	?>
	<br>
	<input type="submit" class="btn btn-default" value="Update" formmethod="post"> <a href="foodinventory.php"><input type="button" name="back" class="btn btn-default" value="Back"></a></span>
	<?php
}
