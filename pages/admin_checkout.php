			...
			if($_REQUEST['confbut'] == "YES")
				{
					...
					$sqls="UPDATE tblreserve SET fldtotal = '$_REQUEST[wow1]', fldpayment = '$_REQUEST[wow1]', fldbalance = '$_REQUEST[balance1]', fldchange = '$_REQUEST[change]'  WHERE RegNo = '$_REQUEST[reg]'";
					mysql_query($sqls);
					
					
					
					header("location:payment_ok.php?reg=$_REQUEST[reg]&food=$_REQUEST[food]&form=1&reserve=$_REQUEST[reserve]&fname=$_REQUEST[fname]&lname=$_REQUEST[lname]&checkin=$_REQUEST[checkin]&checkout=$_REQUEST[checkout]&adult=$_REQUEST[adult]&child=$_REQUEST[child]&types=$_REQUEST[types]&night=$_REQUEST[night]&roomtype=$_REQUEST[roomtype]&roomtypes2=$_REQUEST[roomtypes2]&roomtypes3=$_REQUEST[roomtypes3]&roomtypes4=$_REQUEST[roomtypes4]&roomtypes5=$_REQUEST[roomtypes5]&cabana=$_REQUEST[cabana]&cabana2=$_REQUEST[cabana2]&cabana3=$_REQUEST[cabana3]&cabana4=$_REQUEST[cabana4]&cabana5=$_REQUEST[cabana5]&room=$_REQUEST[room]&room3=$_REQUEST[room3]&room4=$_REQUEST[room4]&payment=$_REQUEST[payment]&numroom=$_REQUEST[numroom]&numrooms=$_REQUEST[numrooms]&numroomss=$_REQUEST[numroomss]&total=$_REQUEST[total]&pay=$_REQUEST[pay]&payment=$_REQUEST[payment]&contact=$_REQUEST[contact]&bal=$_REQUEST[bal]");
					}
					else
					{
					header("location:admin_checkout.php?pr=$_REQUEST[pr]&nice=$_REQUEST[nice]&de=$_REQUEST[de]&payy=$_REQUEST[payy]&food=$_REQUEST[food]&err=$_REQUEST[err]&balan=$_REQUEST[balan]&status=$_REQUEST[status]&type=$_REQUEST[type]&term=$_REQUEST[term]&contact=$_REQUEST[contact]&lname=$_REQUEST[lname]&fname=$_REQUEST[fname]&user=$_REQUEST[user]&pays=$_REQUEST[pays]&payy=$_REQUEST[payy]&reg=$_REQUEST[reg]&form=1&bal=$_REQUEST[bal]&reserve=$_REQUEST[reserve]&checkin=$_REQUEST[checkin]&checkout=$_REQUEST[checkout]&adult=$_REQUEST[adult]&child=$_REQUEST[child]&roomtype=$_REQUEST[roomtype]&types=$_REQUEST[types]&cabana=$_REQUEST[cabana]&room=$_REQUEST[room]&extra=$_REQUEST[extra]&numroom=$_REQUEST[numroom]&balance=$_REQUEST[balance]&total=$_REQUEST[total]&payment=$_REQUEST[payment]&status=$_REQUEST[status]&change=$_REQUEST[change]&downpayment=$_REQUEST[downpayment]&balance1=$_REQUEST[balance1]&pays=$_REQUEST[pays]&conf=1");
					}	
			mysql_close($con);
			}
...
$results = mysql_query("SELECT * FROM tblequipmentinventory");
$results1 = mysql_query("SELECT * FROM tblfoodinven");


$resultss = mysql_query("SELECT * FROM tblequipmentinventory WHERE flditemname = '$_REQUEST[de]'");
$flds = mysql_fetch_array($resultss);

$resultss1 = mysql_query("SELECT * FROM tblfoodinven WHERE flditemname = '$_REQUEST[food]'");
$flds1 = mysql_fetch_array($resultss1);
							...
							$wow = $balan + $flds1['fldprice'] + $flds['fldprice'] + $_REQUEST['pr'];
							$wow1 = $total + $flds1['fldprice'] + $flds['fldprice'] +$_REQUEST['pr'];
...
<form action="admin_checkout.php?reg=<?php echo $_REQUEST['reg'];?>&vnz=1"  method="post">
		<input type="hidden" name="form" value="1">
	<div class="row">
                      <div class="col-lg-12">
					  <div id="white" style="width: 940px; height: 1250px; background-color: white;">
			...
			<?php ///////////////// POST ?>					
						...
						<tr><td align="center">Status: </td><th><?php echo $status; ?></th></tr>
							...
							<font face="century gothic" color="#f3be4b" size="5"> Payment</font>
								...
								<input type="hidden" class="textarea" name="pr"  value="<?php echo $pr;  ?>" ><br><br>

							Additional Person: <input type="text" class="textarea" name="nice"  value="<?php echo $_REQUEST['nice'];  ?>" ><br><br>
							<br><Br>
							Food: <select name="food" class="textarea" onchange="this.form.submit();">
							<option value="other" <?php echo isset($_REQUEST["food"]) && $_REQUEST["food"] == "other" ? "selected" : "other" ?>>Please Select..</option>
							<?php
							while($list = mysql_fetch_assoc($results1))
							{
							?>
							<option value="<?php echo $list['flditemname'];?>" <?php echo isset($_REQUEST["food"]) && $_REQUEST["food"] == "$list[flditemname]" ? "selected" : "$list[flditemname]" ?>><?php echo $list['flditemname'];?></option>
							<?php
							} 
							?>
							</select><br><br>
							
							Price: ₱<input type="text" class="textarea" name="ad"  value="<?php echo $flds1['fldprice'];  ?>"><br><br>
							Equipment (Damaged): <select name="de" class="textarea">
							<option value="other" <?php echo isset($_REQUEST["de"]) && $_REQUEST["de"] == "other" ? "selected" : "other" ?>>Please Select..</option>
							<?php
							while($list = mysql_fetch_assoc($results))
							{
							?>
							<option value="<?php echo $list['flditemname'];?>" <?php echo isset($_REQUEST["de"]) && $_REQUEST["de"] == "$list[flditemname]" ? "selected" : "$list[flditemname]" ?>><?php echo $list['flditemname'];?></option>
							<?php
							} 
							?>
							</select><br><br>
							Equipment Price:  ₱<input type="text" class="textarea" name="ep"  value="<?php echo $flds['fldprice'];  ?>"><br><br>
				
								
						 <input type="hidden" readonly class="form-control" style="width:100%" name="wow" value="<?php echo $wow; ?>"></th></tr>

						<input type="hidden" readonly class="form-control" style="width:100%" name="balan" value="<?php echo  $balan; ?>">
						<tr><td align="center"> Balance: ₱</td><th> <input type="text" readonly class="form-control" style="width:100%" name="wow" value="<?php echo $wow; ?>"></th></tr>

						<input type="hidden" readonly class="form-control" style="width:100%" name="wow1" value="<?php echo $wow1; ?>"></th></tr>
						<input type="hidden" readonly class="form-control" style="width:100%" name="total" value="<?php echo $total; ?>">

						
						<tr><td align="center">Total: ₱ </td><th><input type="text" readonly class="form-control" style="width:100%" name="wow1" value="<?php echo $wow1; ?>"></th></tr>
