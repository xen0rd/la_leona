	...
	<div id="body">
		...
		<div class="header">
			<ul>
				...
				<li>
			<label > <font face="century gothic" color="#f3be4b" size="6"> Toiletries </label></font>
				</li>
				<li>
...
$result = mysql_query("SELECT * FROM tbltoiletries ORDER BY flditemcode ASC");
$dis = mysql_query("SELECT * FROM tbltoiletries WHERE fldquantity = 0");
...
echo "	<table>
  ...
  <td align='center' ><a href='addtoiletries.php'><button type='button' class='btn btn-success' name='add'  >Add Supply</button></td> 
 <td  ><a href='roomlist.php'>&nbsp;&nbsp;&nbsp;<button type='button'  class='btn btn-success' name='add'  >Supply Allocation</button></td> 
</font>
...
if($_REQUEST['add']=='Add')
{
      header("location:addtoiletries.php");

} 
if ($_REQUEST['dis']=='Click Here To View')
	{
		$result = mysql_query("SELECT * FROM tbltoiletries WHERE fldquantity = 0 ORDER BY flditemcode ASC");
	}

if($_REQUEST['search']=='Search')
{
	$result = mysql_query("SELECT * FROM tbltoiletries WHERE flditemcode LIKE '%$buttontext%' OR flditemname LIKE '%$buttontext%' OR fldquantity LIKE '%$buttontext%' ORDER BY flditemcode ASC");
}
...
else
{
 ...
 echo "<td align='center' width=90 class='tdpos'>" . $row['fldprice'] . "</td>";
 echo "<td align='center' width=90 class='tdpos'>" . $row['fldunit'] . "</td>";
?>
 <td align="center"  class='tdpos'><button type='button' class="btn btn-success btn-primary" onclick="window.location.href='deletetoiletries.php?del=<?php echo $row['flditemcode'];?>'" class='text'><i class="fa fa-trash"></i>
  </button></td>
     <td align="center"  class='tdpos'><button type='button' class="btn btn-success btn-primary" onclick="window.location.href='updatetoiletries.php?prodcode=<?php echo $row['flditemcode'];?>'" class='text'><i class="fa fa-edit"></i>
	 </button></td>
  <?php echo "</tr>";
}
