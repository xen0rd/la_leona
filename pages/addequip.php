...
if(empty($_REQUEST['itemname'])||empty($_REQUEST['quantity']))
{
	...
	header("location:addequip.php?err=$err&itemname=$_REQUEST[itemname]&itemnameErr=$itemnameErr&quantity=$_REQUEST[quantity]&quanErr=$quanErr&status=$_REQUEST[status]&statusErr=$statusErr");
	}
	else
	{
	...
	$dupe = mysql_query("SELECT * FROM tblequipmentinventory WHERE flditemname='$_POST[itemname]'") or die (mysql_error());
	$num_rows = mysql_num_rows($dupe);
		if ($num_rows > 0) 
		{
			...
			$prodcodeErr="*";
		 	header("location:addequip.php?err=$err&itemname=$_REQUEST[itemname]&itemnameErr=$itemnameErr");
		}
		else
	...
	
		{
			...
			{
			$sql="INSERT INTO tblequipmentinventory (flddate,flditemname,fldprice,fldquantity)
			VALUES
			('$_REQUEST[date]','$_REQUEST[itemname]','$_REQUEST[price]','$_REQUEST[quantity]')";
			mysql_query($sql,$con);
			header("location:equipinven.php");
			}
			else
			{
		 	header("location:addequip.php?&date=$_REQUEST[date]&itemname=$_REQUEST[itemname]&price=$_REQUEST[price]&quantity=$_REQUEST[quantity]&conf=1");
			}
		}
	}
}
...
<br><br>
<center>
<label > <font face="normal" color="grey" size="4">Date Purchase <font color="red">   </font><input  type="date" class="form-control"  name="date" value="<?php echo $_REQUEST['date']; ?>"></label></font></span><br>
<label > <font face="normal" color="grey" size="4">Equipment Name<span class="colorr"><?php echo $_REQUEST['itemnameErr']; ?> </span></label></font></span><br>
<input type="text" name="itemname" class="form-control" style="width:40%" value="<?php echo $_REQUEST['itemname']; ?>"><br>
<label > <font face="normal" color="grey" size="4">Price</label></font><br>
<input type="text" name="price" class="form-control" style="width:40%" value="<?php echo $_REQUEST['price']; ?>"><br>
...
<br> 
<br>
<input type="submit" class="btn btn-default" value="Submit"  formmethod="post"> <a href="equipinven.php"><button type="button" class="btn btn-default">Back</button></a></span>
</form>
...
if($_REQUEST['conf']==1)
{
	...
	<a href="equipinven.php"><button type="button" class="btn btn-default">NO</button></a></span>
	</div>		
	<?php
}
?>
