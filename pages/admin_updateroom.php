...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	mysql_select_db("dbresort", $con);
					...
					{
					$sql="UPDATE tblroom SET fldtype = '$_REQUEST[type]', fldday= '$_REQUEST[day]', fldnight = '$_REQUEST[night]', fldmaxpax = '$_REQUEST[pax]', fldavailable = '$_REQUEST[total]' 	WHERE fldtype = '$_REQUEST[reg]'";
					mysql_query($sql);
					header("location:manageroom2.php");
					}
					else
					{
					header("location:admin_updateroom.php?reg=$_REQUEST[reg]&form=1&type=$_REQUEST[type]&day=$_REQUEST[day]&night=$_REQUEST[night]&pax=$_REQUEST[pax]&total=$tot&add=$add&delete=$delete&conf=1");
					}
...
$reg=$_REQUEST['reg'];
	$add = $_REQUEST['add'];
	$delete = $_REQUEST['delete'];
		$rest = mysql_query("SELECT fldavailable FROM tblroom WHERE fldtype = '$reg' ");
...
$res = mysql_query("SELECT * FROM tblroom WHERE fldtype = '$reg' ");
...
<form action="admin_updateroom.php" method="post">
...
<input type="hidden" name="reg" value="<?php echo $_REQUEST['reg']; ?>">
 ...
 <label > <font face="normal" color="grey" size="4">Room Type</font></label> <input  type="text"  class="form-control" style="width:40%" name="type" value="<?php echo $type; ?>"><br>
...
if($_REQUEST['conf']==NULL)
{
	?>
	<br>
	<input type="submit" class="buttons" value="Update" formmethod="post"> <a href="manageroom2.php"><input type="button" name="back" class="buttons" value="Back"></a></span>
	<?php
}
