...
if(empty($_REQUEST['roomnumber'])||empty($_REQUEST['roomname']))
{
	...
	header("location:addroomlist.php?err=$err&roomnumber=$_REQUEST[roomnumber]&roomnumberErr=$roomnumberErr&roomname=$_REQUEST[roomname]&roomnameErr=$roomnameErr");
	}
	else
	{
	...
	$dupe = mysql_query("SELECT * FROM roominfo WHERE fldroomnumber='$_POST[roomnumber]'") or die (mysql_error());
	$num_rows = mysql_num_rows($dupe);
		if ($num_rows > 0) 
		{
			...
			$prodcodeErr="*";
		 	header("location:addroomlist.php?err=$err&roomnumber=$_REQUEST[roomnumber]&roomnameErr=$roomnameErr");
		}
		else
	...
	
		{
			...
			{
			$sql="INSERT INTO roominfo (fldroomnumber, fldroomname)
			VALUES
			('$_REQUEST[roomnumber]','$_REQUEST[roomname]')";
			mysql_query($sql,$con);
			header("location:roomlist.php");
			}
			else
			{
		 	header("location:addroomlist.php?&roomname=$_REQUEST[roomname]&roomnumber=$_REQUEST[roomnumber]&conf=1");
			}
		}
	}
}
...
<form action="addroomlist.php" method="post">
<div class="">
<br><br>
<center>
<label > <font face="normal" color="grey" size="4">Room Number<span class="colorr"><?php echo $_REQUEST['roomnumberErr']; ?> </span></label></font></span><br>
...
if($_REQUEST['conf']==1)
{
	...
	<a href="addroomlist.php"><button type="button" class="btn btn-default">NO</button></a></span>
	</div>		
	<?php
}
?>
