...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$rest = mysql_query("SELECT fldquantity FROM tblequipmentinventory WHERE flditemcode = '$prodcodes' ");
...
if(empty($_REQUEST['itemcode'])||empty($_REQUEST['itemname']))
{
	...
	header("location:updateequip.php?prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemcodeErr=$itemcodeErr&itemname=$_REQUEST[itemname]&itemnameErr=$itemnameErr&add=$add&delete=$delete");
}
else
{
	if($tot > 500)
	{
		...
		header("location:updateequip.php?stocksErr=$stocksErr&prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemname=$_REQUEST[itemname]&add=$add&delete=$delete");
	}
	else if($tot <= 500 )
	{
		$tot = $tot - $delete;
		...
		else
		{
			if($_REQUEST['itemcode']==$_REQUEST['prodcode'])
			{
				...
				{
				$sql="UPDATE tblequipmentinventory SET flditemcode = '$_REQUEST[itemcode]', flditemname = '$_REQUEST[itemname]', fldquantity = '$tot' WHERE flditemcode = '$_REQUEST[prodcode]'";
				mysql_query($sql);
				header("location:equipinven.php");
				}
				else
				{
					header("location:updateequip.php?prodcode=$_REQUEST[prodcode]&form=1&itemcode=$_REQUEST[itemcode]&itemname=$_REQUEST[itemname]&&add=$add&delete=$delete&conf=1");
				}
			}
			else
			{
				$dupe = mysql_query("SELECT * FROM tblequipmentinventory WHERE flditemcode='$_POST[itemcode]'") or die (mysql_error());
				...
				if ($num_rows > 0) 
				{
					...
					$itemcodeErr="*";
					header("location:updateequip.php?prodcode=$_REQUEST[prodcode]&form=1&err=$err&itemcode=$_REQUEST[itemcode]&itemcodeErr=$itemcodeErr&itemname=$_REQUEST[itemname]&add=$add&delete=$delete");
				}
				else 
				{
					if($_REQUEST['confbut'] == "YES")
...
$prodcode=$_REQUEST['prodcode'];
	...
	mysql_select_db("dbresort", $con);
$res = mysql_query("SELECT * FROM tblequipmentinventory WHERE flditemcode = '$prodcode' ");
...
<div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
<form action="updateequip.php" method="post">
<input type="hidden" name="form" value="1">
<input type="hidden" name="prodcode" value="<?php echo $_REQUEST['prodcode']; ?>">

<br><br>
<center>
 <label > <font face="normal" color="grey" size="4">Equipment No.:</font></label> <input readonly type="text"  class="form-control" style="width:40%" name="itemcode" value="<?php echo $itemcode; ?>"><span class="colorr"><?php echo $_REQUEST['itemcodeErr']; ?></span><br>
<label > <font face="normal" color="grey" size="4">Equipment Name:</font></label> <input type="text" class="form-control" style="width:40%" name="itemname" value="<?php echo $itemname; ?>"><span class="colorr"><?php echo $_REQUEST['itemnameErr']; ?></span><br>
...
if($_REQUEST['conf']==NULL)
{
	?>
	<br>
	<input type="submit" class="buttons" value="Update" formmethod="post"> <a href="equipinven.php"><input type="button" name="back" class="buttons" value="Back"></a></span>
	<?php
}
...
{	
	...
	<a href="equipinven.php"><button type="button"  class='btn btn-success'>NO</button></a></span>
	</div>
	<?php
}
