		...
		if ($_FILES["file"]["error"] > 0)
			{
				...
				$sql="UPDATE tblgallery set description='$desc', fname='images/".$_FILES["file"]["name"]."' where id=$id";
				//echo $sql;
				$result = mysql_query($sql);
				header("Refresh:1; url=gallery-maintenance.php");
			}
	}else{
		//echo "<script>alertify.error('Invalid file detail')</script>";
		$sql="UPDATE tblgallery set description='$desc' where id=$id";
		//echo $sql;
		$result = mysql_query($sql);
		header("Refresh:1; url=gallery-maintenance.php");
	}
