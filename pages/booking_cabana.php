...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if(empty($_REQUEST['checkin'])||empty($_REQUEST['checkout'])||$_REQUEST['types']=="other")
	{
		...
		header("location:booking_cabana.php?err=$err&checkin=$_REQUEST[checkin]&checkout=$_REQUEST[checkout]&night=$_REQUEST[night]&types=$_REQUEST[types]&inErr=$inErr&outErr=$outErr&typesErr=$typesErr");
	...
	else
	{
		if ($_REQUEST['confbut'] == "Check Availability")
		{
			...
			if ($_REQUEST['checkin'] <= $tomorrow)
			{
				...
				$outErr = "*";
				$inErr = "*";
				header("location:booking_cabana.php?err=$err&checkin=$_REQUEST[checkin]&checkout=$_REQUEST[checkout]&night=$_REQUEST[night]&types=$_REQUEST[types]&checkawt=$_REQUEST[checkawt]");
			...
			{
			header("location:booking_cabana1.php?err=$err&checkin=$_REQUEST[checkin]&checkout=$_REQUEST[checkout]&night=$_REQUEST[night]&types=$_REQUEST[types]&checkawt=$_REQUEST[checkawt]");
			}
		}
	}
}
...
<span id="tab"><p>- easy access to the pool area.
		...
		<hr>
						...
						<form action="booking_cabana.php" method="get">
